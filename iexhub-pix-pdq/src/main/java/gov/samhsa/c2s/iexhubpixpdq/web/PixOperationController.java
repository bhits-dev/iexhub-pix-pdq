...
public class PixOperationController {
    ...
    @RequestMapping(value = "/Patient", consumes = IexhubPixPdqProperties.Fhir.MediaType.APPLICATION_FHIR_JSON_UTF8_VALUE)
    @ResponseStatus(HttpStatus.CREATED)
    public void addPerson(@RequestBody FhirPatientDto fhirPatientDto) {
        pixOperationService.registerPerson(fhirPatientDto);
    }

    @RequestMapping(value = "/Patient/{id}", consumes = IexhubPixPdqProperties.Fhir.MediaType.APPLICATION_FHIR_JSON_UTF8_VALUE)
    @ResponseStatus(HttpStatus.OK)
